...
export let postLogin = (req: Request, res: Response, next: NextFunction) => {
    ...
    passport.authenticate("local", (err: Error, user: UserModel, info: IVerifyOptions) => {
        ...
        if (user.confirmationStatus !== ConfirmationStatus.Approved) {
            handleConfirmationStatus(user.confirmationStatus, req, res);
            return;
        }
        req.logIn(user, (err) => {
            if (err) {
...
function handleConfirmationStatus(confirmationStatus: ConfirmationStatus, req: Request, res: Response) {
    switch (confirmationStatus) {
        case ConfirmationStatus.Pending: {
            req.flash("info", { msg: "Your request is yet to be approved, you will be able to login once it will be approved by an admin." });
            res.redirect(req.session.returnTo || "/");
            break;
        }

        case ConfirmationStatus.Declined: {
            req.flash("errors", { msg: "Your request to sign up has been declined by an admin." });
            res.redirect(req.session.returnTo || "/");
            break;
        }
    }
...
export let postSignup = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (errors) {
        // server side rendering placeholder, remove in the future
        req.flash("errors", errors);
        return res.redirect("/signup");
    }

    // create a new user with a pending confirmation status
